...
15분마다 (또는 수동 실행)
  ...
  → Cloudflare Workers AI (IBM Granite) 로 한국어 초안 작성
  → src/posts/YYYY-MM-DD-devlog.md 에 draft: true 로 저장
...
4. **기준점 잡기 (선택)** — 아무것도 안 하면 첫 실행이 **최근 90일치 푸시를
   전부** 글감으로 잡습니다(저장소 9곳, 커밋 100건 넘게). 지난 것은 넘기고
   앞으로의 푸시만 쓰고 싶으면:
   ...
   과거를 다 쓰고 싶다면 그냥 두면 됩니다. 한 글에 저장소 5곳까지만 담고
   나머지는 다음 실행으로 이어지므로 여러 편에 나눠 실립니다.
