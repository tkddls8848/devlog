...
// 프로세스로 돌리고 표의 소스와 Cloudflare AI의 네트워크만 스텁해, 저장한
...
test("여러 소스의 소식을 한 편으로 묶어 발행한다", () => {
  const result = run({
    ...
    feeds: {
      [BYLINE]: { items: [story("바이라인 기사", "https://byline.network/1")] },
      [REGISTER]: { items: [story("Register 기사", "https://www.theregister.com/2026/08/18/1")] },
    },
  });
  ...
  assert.match(result.issue, /source: "Byline Network"\n {4}kind: "국내 미디어"/);
  assert.match(result.issue, /url: "https:\/\/byline\.network\/1"/);
...
test("표에 있는 소스는 모두 수집을 시도한다", () => {
  ...
  const result = run({ feeds: { [BYLINE]: { items: [story("바이라인 기사", "https://byline.network/1")] } } });

  for (const name of ["Hacker News", "Byline Network", "GeekNews", "토스 테크", "The Register"]) {
...
test("발행한 이슈에 담긴 링크는 다시 싣지 않는다", () => {
  ...
  const result = run({
    ...
    feeds: { [BYLINE]: { items: [story("바이라인 기사", "https://www.byline.network/1?utm_source=rss")] } },
  });
...
test("한 수집에서 중복으로 올라온 글은 하나만 담는다", () => {
  const result = run({
    feeds: {
      [TOSS]: {
        items: [
          story("토스 글", "https://toss.tech/article/a"),
          story("토스 글 재발행", "https://toss.tech/article/a?utm_medium=feed"),
        ],
      },
    },
  });
...
test("한 소스가 뉴스레터를 다 채우지 못하게 자른다", () => {
  const many = Array.from({ length: 9 }, (_, index) =>
    story(`바이라인 ${index}`, `https://byline.network/${index}`, index + 1)
  );
  const result = run({
    feeds: {
      [BYLINE]: { items: many },
  ...
  assert.equal(result.state.picked["Byline Network"], 3);
...
test("전체 개수 상한을 넘으면 최신 글부터 남긴다", () => {
  const result = run({
    feeds: {
      [BYLINE]: {
        items: [
          story("가장 새 글", "https://byline.network/1", 1),
          story("중간 글", "https://byline.network/2", 5),
        ],
      },
...
test("AI가 실패해도 링크 목록으로 뉴스레터를 낸다", () => {
  const result = run({
    feeds: { [BYLINE]: { items: [story("바이라인 기사", "https://byline.network/1")] } },
    ai: { fail: true },
  });
  ...
  assert.match(result.issue, /\[바이라인 기사\]\(https:\/\/byline\.network\/1\)/);
});
...
test("글을 하나도 못 받은 소스는 구조 변경을 의심하라고 경고한다", () => {
  ...
  assert.match(result.output, /GeekNews가 글을 하나도 돌려주지 않았습니다/);
...
test("드라이런은 담을 소식을 보여 주고 저장하지 않는다", () => {
  ...
  assert.equal(result.status, 0);
  assert.match(result.output, /Byline Network: 1건/);
  assert.match(result.output, /- 바이라인 기사/);
